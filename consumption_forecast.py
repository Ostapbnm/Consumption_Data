...
def load_historical_data():
    ...
    # Формат дати в hour_start у різних завантажень репо виявився різним:
    # то 'DD.MM.YYYY H:MM' (dayfirst), то 'YYYY-MM-DD HH:MM:SS' (ISO). Якщо
    # завжди форсувати dayfirst=True, ISO-рядки на кшталт '2026-08-13' pandas
    # намагається прочитати як %Y-%d-%m (день=08, місяць=13) і падає, бо
    # місяця 13 не існує. Тому спершу пробуємо строгий ISO8601-парсинг,
    # і лише якщо він не підходить - dayfirst-евристику для нестандартних форматів.
    try:
        df['Дата'] = pd.to_datetime(df['hour_start'], format='ISO8601')
